...
def build_brain_spec():
    """
    Builds the initial brain specification (genotype) with REDUCED CONNECTION WEIGHTS and LOW NOISE.
    ...
    # Sensory inputs remain fully reliable (1.0 weight)
    sensory_mapping = {
    ...
    # ---------------------------------
    # column 0: threshold
    # column 1: noise level (set to 0.1 for low noise)
    # column 2: tonic
    neuron_params = np.zeros((n_neurons, 3), dtype=float)
    neuron_params[:, 0] = 0.5   # threshold
    neuron_params[:, 1] = 0.1   # noise - low noise level
    ...
    # All neuron-to-neuron connections now have weight 0.6 (reduced from 1.0)
    
    # neurons 0-4 feed to neurons 5-9 with weight 0.6
    # AND neurons 0-4 inhibit neuron 10 with weight -0.6
    for i in range(5):
        src = i
        tgt = i + 5
        connections[src, tgt, 0] = 0.6  # weight (reduced)
        connections[src, 10, 0] = -0.6  # inhibition to interneuron (reduced)

    # interneuron 10 excites output neurons 6-9 with weight 0.6 (but not 5 which is "stay")
    for i in range(6, 10):
        connections[10, i, 0] = 0.6
